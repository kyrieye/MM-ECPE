...
class Transformer_emo(nn.Module):
    ...
    def __init__(
            self, d_model, d_inner,d_outer, emo_num, dropout=0.1):

        super(Transformer_emo, self).__init__()

        self.emo_dec = ScaledDotProductAttention(temperature=np.power(1408,0.5))

        self.fc = nn.Linear(d_model, d_inner)        
        self.classifier = nn.Linear(d_inner, 34)

        self.emo_dec2 = ScaledDotProductAttention(temperature=np.power(1408,0.5))
    ...
    def forward(self, src_seq, trg_seq, src_seq_34):
        ...
        enc_vt, attn, energies= self.emo_dec(trg_seq, src_seq_34,src_seq_34)
        ...
        src_mask = torch.gather(top_mask, 1, dummy).repeat(1,trg_seq.shape[1],1)


        enc_vt, attn, energies = self.emo_dec2(trg_seq, src_seq,src_seq, mask=src_mask)
